...
import { useMousetrap } from '../src';

# useMousetrap

The `useMousetrap` hook follows the hotkeys call signature.
...
const AddToBankComponent = () => {
  ...
  useMousetrap('ctrl+a', () => setAmount(prevAmount => prevAmount + 100));
  useMousetrap('ctrl+d', () => setAmount(prevAmount => prevAmount - 100));
...
const AddToBankComponent = () => {
  ...
  useMousetrap('n', () => setAmount(amount + 100), []);
...
case it's the `amount` variable. Our `() => setAmount(amount + 100)` callback gets memoised inside the `useMousetrap` hook
...
the first time it got executed. So inside the `useMousetrap` hook the value of `amount` will always be `0`, which is the
...
const AddToBankComponent = () => {
  ...
  useMousetrap('m', () => setAmount(amount + 100), [amount]);
...
const AddToBankComponent = () => {
  ...
  useMousetrap('m', () => setAmount(amount => amount + 100), []);
